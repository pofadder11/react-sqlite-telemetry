...
export default function FleetCanvas() {
            ...
            trailsRef.current.forEach((trail, ship) => {
                ...
                if (active) {
                    ...
                    if (mousePx) {
                        ...
                        if (!nearest || d2 < nearest.d2) nearest = { ship, px: p.x, py: p.y, d2 };
                    }
                }
            });

            // ship hover label
            if (nearest && nearest.d2 < 16 * 16) {
                setHoverShip(nearest.ship);
                drawTooltip(ctx, nearest.px + 10, nearest.py - 6, [nearest.ship]);
            } else {
                setHoverShip(null);
            }
    ...
    }, [mouse, showLabels, wpCount, highlights]);
